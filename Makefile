...
test-int-pg: ## integration: Postgres-узлы/миграции/multi-tenancy
	$(GO) test -tags=integration -count=1 -v -timeout $(INTEGRATION_TIMEOUT) -run "^TestNodeRepo|^TestNodeUC|^TestNodeCache|^TestNodeSearch|^TestMigrations|^TestMultiTenancy" ./tests/integration/...
...
test-int-catalog: ## integration: каталоги/auth/сессии/нотификации/circuit-breaker
	$(GO) test -tags=integration -count=1 -v -timeout $(INTEGRATION_TIMEOUT) -run "^TestHostAllowlist|^TestHeaderCatalog|^TestAuth|^TestSession|^TestUserRoleManager|^TestSearchHistory|^TestAppSettingsRepo|^TestNotif|^TestCircuitBreaker" ./tests/integration/...

...
- **§81 — защита узла (circuit breaker): настраиваемая политика и ручной сброс.** Миграция
  ...
  **Порядок выката строгий: Sender → Receiver → Web.** Политика узла доезжает до синхронного пути в
  gRPC-запросе (новые поля `circuit_breaker_threshold` / `circuit_breaker_cooldown_sec` в
  `SendRequest`). Старый Sender эти поля игнорирует и работает по глобальной политике — это
  безопасно; обратный порядок (сначала Receiver) на время выката просто не даст per-node
  переопределению действовать на sync-узлах.

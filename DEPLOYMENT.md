...
- **§71 — персональные предпочтения (дефолтный период рабочего стола per-team).** Миграция
  ...
  **Требуется PostgreSQL ≥ 15** — используется `UNIQUE NULLS NOT DISTINCT` (в поставке 16; при
  внешнем кластере версию проверить заранее: на PG 14 миграция не применится и сервис не поднимется).

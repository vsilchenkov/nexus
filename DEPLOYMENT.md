...
2. **Прячет на `127.0.0.1` всю инфраструктуру.** Корневой `docker-compose.yml` выставляет
   ...
   на `0.0.0.0` — на публичном IP это открытые наружу хранилища. Снаружи остаются только
   `web:8000` (панель) и `receiver:8080` (приём запросов шины) — без них в систему не зайти,
   а клиенты не смогут слать запросы.
...
Ожидаемый результат — 10 строк, из них ровно две наружу: `web` (8000) и `receiver` (8080).
Любая другая строка с `0.0.0.0` — дефект конфигурации, хранилище смотрит в интернет.

**Привязка точек входа — `WEB_BIND` / `RECEIVER_BIND`.** По умолчанию `0.0.0.0`, то есть панель
и приём запросов доступны сразу после `up -d`. Это открытый **HTTP**: сессионная кука и токены
команд идут по сети незашифрованными, поэтому на публичном сервере правильная схема — nginx/caddy
на хосте слушает 443 с TLS и проксирует внутрь. Когда reverse-proxy поднят, допишите в `.env`:
...
и порты уйдут на loopback без правки compose-файлов. `RECEIVER_BIND=127.0.0.1` уместен и сам по
себе, если все клиенты шины живут на этом же хосте — тогда 8080 наружу не нужен вовсе.
...
- **§88 — почта (SMTP) и восстановление пароля.** Миграция `0036_one_time_tokens` создаёт
  ...
  **Настройки почты миграции не требуют** — они лежат в `app_settings.value` (JSONB), как
  ...
  `config.yml` от §88 появляется единственный ключ — лимит частоты запросов восстановления
  (см. раздел про конфигурацию).

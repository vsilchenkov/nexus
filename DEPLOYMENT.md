...
> ⚠️ **Обновляя стек с балансировщиком, НЕ исключайте `nginx` из выката.**
...
> вход лёг). Полный `docker compose up -d --build` (без фильтра сервисов) пересоздаёт
> nginx сам; если обновляете по списку — включайте в него `nginx`. HA-стек (§5.5)
> обновляется через `rolling.sh`, который делает reload сам.

...
> **Два лимита тела, а не один.** `receiver.max_body_bytes` (124 МиБ) ограничивает ТОЛЬКО
> sync-путь `/api/v1/request/*`: тело идёт напрямую по gRPC, очередь не участвует.
> `receiver.max_async_body_bytes` (32 МиБ) ограничивает всё, что уезжает в Kafka —
...
> размер сообщения брокера, а не в память Receiver'а. Значение `0` → `min(32 МиБ,
...
> 3. **Существующие топики.** Нужно только если поднимается `max_async_body_bytes`:
>    `kafka.topic.*` применяется лишь при создании топика, поэтому живые `nexus.async`,
>    `nexus.async.dlq`, `nexus.async.paused`, `nexus.logs.retry` остаются со старым
>    `max.message.bytes` — поднимите его `kafka-configs --alter` (команда ниже).
> 4. **Таймаут чтения.** `receiver.read_timeout_ms` включает приём тела: 124 МиБ на медленном
>    канале не влезают в прежние 10 с, клиент получает обрыв, а в логах шины запроса нет вовсе.
>    В поставке 300000 (5 мин).
> 5. **Память сервисов.** Тело sync-запроса лежит в RAM целиком — и в Receiver, и в Sender.
>    Замер на стенде (124 МиБ): Receiver RSS ~690 МиБ, Sender ~869 МиБ на один запрос; три
>    параллельных — Sender 1.36 ГиБ. Лимиты в `deploy/docker-compose.override.yml` подняты
>    (receiver 1g, sender 1792m); при большем потоке крупных тел считайте
>    ~2.5 × `max_body_bytes` × число одновременных предельных запросов.
>
> Sync-путь ограничен `receiver.max_body_bytes` (124 МиБ) и gRPC-лимитом (192 МиБ, обе стороны
> обязаны совпадать); async — `receiver.max_async_body_bytes` (32 МиБ) и Kafka: тело едет в
> конверте base64 (+33%), то есть 32 МиБ тела ≈ 42.6 МиБ сообщения.
>
...
client_max_body_size 140m;    # ≥ receiver.max_body_bytes (124 МиБ) + запас на заголовки
```
...
> gRPC, Sender — тело запроса и тело ответа апстрима одновременно. При `max_body_bytes` 124 МиБ
...
крупные тела (`receiver.max_body_bytes` — 124 МиБ), поднимите лимиты `receiver`
...
`mem_limit` не делятся, а удваиваются вместе с репликами. При работе с телами до 124 МиБ это
6 ГБ потолков приложения вместо 3 — планируйте 8 ГБ RAM либо снижайте лимиты в

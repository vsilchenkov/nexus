...
- **§82 — потолок длительности sync-вызова и гейт async-входа. Миграций нет.**
  ...
  **Порядок выката: Sender → Receiver.** Sender должен получить keepalive-политику раньше, чем
  Receiver начнёт вести к нему долгие вызовы; обратный порядок не ломает ничего, просто оставляет
  прежний потолок до обновления Sender'а. Web можно обновлять когда угодно.

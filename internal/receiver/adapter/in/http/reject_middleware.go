...
// RejectProxiedKey — ключ gin-контекста: ответ отдан ВНЕШНЕЙ системой, шина
// его лишь пробросила.
//
// Нужен потому, что статус ответа шины и статус решения шины — разные вещи.
// Узел с path_passthrough (§39) отдаёт клиенту ровно то, чем ответил апстрим:
// Telegram на editMessageReplyMarkup штатно отвечает 400 «message is not
// modified», Green API — 400 на неизвестный идентификатор. Для §94 это НЕ
// отказ: шина отработала правильно, запрос дошёл, ответ доставлен, и он уже
// записан в лог узла со всем содержимым.
//
// Найдено на бою 23.08.2026: в журнале отказов появились группы
// telegram/bot…/editMessageReplyMarkup и green/whatsapp/…/getAvatar с
// причиной bad_request — то есть успешно обслуженные запросы выглядели как
// отвергнутые шиной.
const RejectProxiedKey = "nexus_reject_proxied"

// MarkProxiedResponse помечает ответ как полученный от внешней системы.
// Вызывается ПЕРЕД записью ответа клиенту — после неё gin уже зафиксировал
// статус, а middleware читает контекст после c.Next().
func MarkProxiedResponse(c *gin.Context) {
	c.Set(RejectProxiedKey, true)
}
...
func rejectReason(c *gin.Context) (domain.RejectReason, bool) {
	...
	// Ответ внешней системы отказом шины не является — какой бы у него ни был
	// статус. Проверка стоит ПОСЛЕ явной причины намеренно: если обработчик
	// назвал причину, она главнее (проброс и отказ одновременно невозможны, но
	// порядок фиксируем, чтобы правило не зависело от порядка вызовов).
	// Ответ внешней системы отказом шины не является — какой бы у него ни был
	// статус. Проверка стоит ПОСЛЕ явной причины намеренно: если обработчик
	// назвал причину, она главнее (проброс и отказ одновременно невозможны, но
	// порядок фиксируем, чтобы правило не зависело от порядка вызовов).
	if c.GetBool(RejectProxiedKey) {
		return "", false
	}
	status := c.Writer.Status()

...
// classifyUpstream различает «приёмник не смог» и «мы перестали ждать».
//
// Воздержание — когда ушла ВЫЗЫВАЮЩАЯ сторона: клиент шины разорвал соединение
// либо сервис останавливается. Приёмник в этот момент может быть совершенно
// здоров — мы просто не дождались ответа. Засчитывать это отказом нельзя:
// боевой инцидент 06.08.2026 (§81.1) — приёмник отвечал 200 за 145 с, клиента
// обрывали на 50-й секунде, пять таких обрывов открывали breaker, а
// половинчато-открытая проба обрывалась ровно так же, и выхода из состояния не
// существовало.
//
...
// оставляет родителя живым (ctx.Err() == nil), а уход вызывающей стороны его
// убивает — и неважно, Canceled это или её собственный DeadlineExceeded.
// Разбор строк ошибок не нужен (CLAUDE.md §5).
//
...
func classifyUpstream(ctx context.Context, resp *port.HTTPResponse, lastErr error) breakerVote {
	...
	case ctx.Err() != nil:
		return voteAbstain // вызывающая сторона ушла
...
func (u *SendUsecase) voteBreaker(ctx context.Context, in SendInput, resp *port.HTTPResponse, lastErr error, rec *domain.LogRecord) {
	...
	if vote == voteAbstain {
		...
		u.logger.Debug("send: caller gone, breaker vote abstained",
			u.logger.Str("id", in.ID),
...
//
// Нормализуются два исхода (§81.2.1): уход вызывающей стороны и собственный
// таймаут узла. Остальное — текст ошибки как есть: он не содержит входных
// данных и нужен для диагностики адреса.
//

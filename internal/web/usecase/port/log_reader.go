...
type LogQuery struct {
	...
	// ExcludeReasonPrefixes — маркеры причин, которые НЕ должны попасть в
	// выборку (§81.5). Сопоставление по началу строки: reason пишется как
	// «<маркер>: <деталь>» (domain.Reason*).
	//
	// Единственный потребитель — массовый повтор: запись, где ушла вызывающая
	// сторона, приёмник, скорее всего, уже обработал (тело он принял целиком),
	// и повтор дал бы дубли — ровно то, что случилось в §79 (15 дублей в 1С на
	// узле kz). Просмотр и очистка «неудачных» такие записи НЕ прячут: шина
	// доставку не подтвердила, и это честный сигнал «клиенты не дожидаются».

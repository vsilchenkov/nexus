...
func newTester(t *testing.T,
	...
	return NewSettingsTester(repo, cfg, chFactory, sentryFactory, nil, nil, "Test", "v0", logging.NewNoop())
}
...
func TestSettingsTester_TestTelegram(t *testing.T) {
	...
	cfg := &config.Config{}
	sender := &notifSender{}
	tester := NewSettingsTester(repo, cfg, nil, nil, sender, nil, "Test", "v0", logging.NewNoop())
...
func TestSettingsTester_TestTelegram_NoToken(t *testing.T) {
	t.Parallel()
	repo := &fakeAppSettingsRepo{current: &domain.AppSettings{}}
	tester := NewSettingsTester(repo, &config.Config{}, nil, nil, &notifSender{}, nil, "Test", "v0", logging.NewNoop())

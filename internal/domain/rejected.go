...
const (
	...
	// RejectedMaxStoredGroups — потолок числа групп в хранилище.
	//
	// Предел на буфер экземпляра (max_groups) защищает только память Receiver:
	// сканер по случайным путям создаёт НОВУЮ группу на каждую попытку, и за
	// срок хранения их накопились бы миллионы строк. Чистка держит самые
	// свежие по last_seen — журнал отвечает на вопрос «кто стучится», а не
	// хранит полную историю мусора.
	RejectedMaxStoredGroups = 20000

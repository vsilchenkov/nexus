...
- **`UNIQUE NULLS NOT DISTINCT` — не украшение, а условие работоспособности.** Глобальный преф
  хранится строкой с `team_id IS NULL`. С обычным `UNIQUE` две такие строки считаются различными
  (`NULL <> NULL`), `ON CONFLICT` для них не срабатывает, и upsert молча превращается в append:
  таблица растёт, а чтение отдаёт произвольную из копий. Отсюда требование **PostgreSQL ≥ 15**
  (в поставке 16) и integration-тест «двойной upsert глобального префа даёт ОДНУ строку» — он
  красный на схеме без `NULLS NOT DISTINCT`.
...
- **Принудительного сброса периода при смене команды в коде нет и быть не должно** (§71.5). Он
  выглядит естественным дополнением («раз период команды — сбросим на него»), но ломает явный выбор
  пользователя и дип-линки. Поведение закреплено тестом «явный `?range=1h` переживает переключение
  команды» — если он покраснел, кто-то добавил такой эффект.

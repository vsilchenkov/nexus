...
Форма узла ([NodeSettings.tsx](../web-ui/src/pages/NodeSettings.tsx)) показывает read-only строку
«Команда: <имя>» **внутри карточки «Предпросмотр»** (первой строкой после заголовка; первая
реализация отдельным блоком над карточкой выбивалась визуально и опускала предпросмотр вниз —
переделано по фидбэку пользователя). Источники разные по режиму: правка —

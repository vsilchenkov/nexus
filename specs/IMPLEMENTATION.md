...
- **Фильтр по группе показывает не справочник, а используемые группы.** Найдено на стенде: в
  команде с двумя группами селект предлагал все шесть, заведённых в инсталляции, и четыре из них
  давали пустой список. Набор считается по узлам скоупа (`nodesQ.data`, ДО клиентских фильтров —
  иначе выбор группы схлопнул бы список опций до неё одной), поэтому в сквозном режиме §86 он
  автоматически получается по всем командам. Выбранная группа остаётся в списке, даже если её узлов
  сейчас не видно, — иначе селект показывал бы чужой пункт при действующем фильтре.

- **Правило «только используемые значения» распространено на «Метод» и «Статус», но не одинаково.**
  Метод и группа — поля узла: известны сразу для всего скоупа, сужаются всегда. Статус вычисляется
  из метрик, и до их прихода все узлы «unknown» — отбор по факту оставил бы «Пауза»/«Отключён», то
  есть «Down» исчезал бы ровно тогда, когда его ищут. Поэтому статус сужается только при
  `metricsReady`, а выбранное значение остаётся в списке в любом случае.
...
### 4.85 §100 — зоны клика: что проверяется только в браузере

Раздел [100-click-targets.md](sections/100-click-targets.md). Ветка `feature/node-groups`. Ниже —
то, чего по коду не видно.

| Пункт | Статус | Где |
|---|---|---|
| Приём (три константы) + строка и карточка узла | ✅ Phase 100.1 | [lib/stretchedLink.ts](../web-ui/src/lib/stretchedLink.ts), [Overview.tsx](../web-ui/src/pages/Overview.tsx) |
| Топы Kafka: строка целиком, подсветка только у ссылок | ✅ Phase 100.2 | [kafka/TopNodes.tsx](../web-ui/src/components/kafka/TopNodes.tsx) |
| Журнал отказов: строка-ссылка, SPA-переход в настройки | ✅ Phase 100.3 | [logs/RejectedTab.tsx](../web-ui/src/pages/logs/RejectedTab.tsx), [lib/rejectedShare.ts](../web-ui/src/lib/rejectedShare.ts) |
| Переходы между вкладками узла | ✅ Phase 100.4 | [lib/nodeTabLink.ts](../web-ui/src/lib/nodeTabLink.ts), [node/OverviewTab.tsx](../web-ui/src/components/node/OverviewTab.tsx), [node/CapacityCard.tsx](../web-ui/src/components/node/CapacityCard.tsx), [node/QueueTab.tsx](../web-ui/src/components/node/QueueTab.tsx) |
| Ссылка со стилями кнопки + репо-широкие стражи | ✅ Phase 100.5 | [ui/buttonClasses.ts](../web-ui/src/components/ui/buttonClasses.ts), [lib/interactiveNesting.guard.test.ts](../web-ui/src/lib/interactiveNesting.guard.test.ts) |

- **Дефект нашёл пользователь, а не тесты — и не мог найти никто другой.** 739 фронт-тестов, вся
  типизация и линтер были зелёными: ссылка существовала, имела правильный `href` и открывалась
  кликом ПО НАДПИСИ. Мёртвым было поле вокруг неё, а поле — это раскладка, которой в jsdom нет
  вовсе. Отсюда вывод для тестов раздела: они охраняют контракт классов и структуру DOM, а зону
  проверяет браузер через `document.elementFromPoint`. Так дефект и был найден, и так же
  проверялось исправление.

- **stopPropagation под растянутой ссылкой бессилен.** Он спасает от всплытия к обработчику предка;
  здесь ссылка перехватывает клик ДО цели — накрытый элемент события не получает, останавливать
  нечего. Единственное лекарство — поднять элемент. Практическое следствие: при каждом растягивании
  нужно перечислить интерактивы строки, включая неочевидные — подсказки `title` и текст, который
  копируют мышью (оверлей ест и hover, и выделение).

- **Опора обязана быть самой внешней.** `relative` на ячейке вместо строки схлопывает зону обратно
  до ячейки, оставив все классы на месте — то есть возвращает исходный дефект в форме, которую
  глазами не отличить. Именно это караулит третья проверка в `expectStretchedTo`.

- **Отказ опоры на `<tr>` был бы катастрофическим и молчаливым.** Псевдоэлементы уехали бы к
  ближайшему позиционированному предку, а без него — к корню страницы: оверлеи всех строк накрыли бы
  экран, и клик в любом месте открывал бы последний узел списка. Поэтому обёртка таблицы объявлена
  второй опорой — отказ схлопывается до одной таблицы. Сам отказ проверен и не воспроизводится:
  `position: relative` на `<tr>` работает в целевых браузерах даже при `border-collapse: collapse`
  (проверено экспериментом в том же Chromium, что и стенд: ссылка в 31 px накрыла строку в 600 px).

- **Playwright оказался вторым подтверждением приёма.** Попытка кликнуть по соседней ячейке строки
  отказа завершилась отказом инструмента: «`<a href=…>` from `<td>` subtree intercepts pointer
  events». Ровно то, чего добивались.

- **Kafka живьём не проверялась** — на стенде нет трафика через Kafka, топы пусты. Покрыта
  юнит-тестами (в том числе «строка без ссылки не подсвечивается и опорой не служит»); зону там
  придётся подтвердить на бою после выката.

- **Колбэки переходов между вкладками убраны, а не заменены.** Ссылке адрес нужен на рендере,
  поэтому его строит сам компонент через `useNodeTabTo()`; `NodeDetail` больше не посредник. У
  «Открыть в логах» это сместило момент вычисления окна с клика на рендер — href остаётся свежим за
  счёт 15-секундного поллинга вкладки «Очередь», и это записано в коде: уберут поллинг — окно
  замрёт.

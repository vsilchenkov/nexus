...
- **Переключение РОВНО ОДИН РАЗ + залипание `ready` — чтобы не воевать с §4.34.** §4.34 намеренно
  ...
  открытие узла (guard по `id`), а достигнув `ready`, статус залипает (`latchedId`): последующая
  ручная смена команды уходит по старому пути §4.34 (детальная 404→редирект; форма — баннер с
  сохранением правок). Узел грузится только при `ready` — иначе team-scoped `GET` отдал бы 404
  ещё до переключения.

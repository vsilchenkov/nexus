...
- **Размеры мержатся ДО записи в Redis-кеш** (`fillTopicSizes` в `KafkaMonitorUsecase.Topics`):
  один instant-запрос на TTL (30 с) и целостный снимок в кеше. Размер меняется медленно, гнаться
  за свежестью нечего.
...
- **Образ Kafka стал собираемым.** `docker compose up -d --build` теперь строит и брокер (нужен
  Maven Central либо свой `--build-arg JMX_AGENT_URL`), а применение требует **пересоздания
  контейнера** — javaagent грузится только при старте JVM, то есть простой брокера 30–60 с. В

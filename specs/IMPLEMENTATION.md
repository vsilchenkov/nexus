...
Форма узла ([NodeSettings.tsx](../web-ui/src/pages/NodeSettings.tsx)) показывает read-only блок
**внутри карточки «Предпросмотр», под схемой маршрута** (за разделителем; первая реализация
отдельным блоком над карточкой выбивалась визуально и опускала предпросмотр вниз, вторая — строкой
над схемой — тоже не устроила; итог по фидбэку: под схемой): строка «Команда: <имя>» + для
сохранённого узла даты §63 в формате «Конфига» («Создано» всегда, «Обновлено» только при
`updated_at ≠ created_at`, пустой автор не выводится; на форме создания дат нет — узла ещё нет).
Источники разные по режиму: правка —

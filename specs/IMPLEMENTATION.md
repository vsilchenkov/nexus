...
### 4.0.6 Два лимита тела: sync 124 МиБ, async 32 МиБ
...
- **Дефолт `min(32 МиБ, max_body_bytes)`** ([defaults.go](../internal/platform/config/defaults.go)):
  конфиг без нового ключа ведёт себя как раньше, а async никогда не принимает больше sync.
...
### 4.80 §94 — журнал отказов: агрегат вместо ленты, и почему запись не может быть надёжной
...
**Лока у чистки пока нет — и это осознанно.** `redislock`/`CycleLock` живут в §93, который
в `dev` ещё не влит. `DELETE` по условию идемпотентен, вторая реплика делает пустой
проход. При слиянии с §93 цикл берётся под лок, но ключи периодического и внеочередного
проходов ДОЛЖНЫ различаться: общий лок с длинным TTL от тикера отложил бы применение
нового срока хранения, то есть сломал бы обещание «уменьшение применяется сразу».

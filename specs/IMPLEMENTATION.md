...
| **Семантика offset'ов async-consumer'а: paused/no-loss, at-least-once, FIFO, multi-instance** | ✅ (Phases 2–5, см. 4.37) | [sender_async_redelivery_test.go](../tests/integration/sender_async_redelivery_test.go) (paused доставляется после unpause без рестарта; сообщение не теряется при commit'е соседа), [sender_async_atleastonce_test.go](../tests/integration/sender_async_atleastonce_test.go) (падение до commit'а → ровно один дубль), [sender_async_fifo_test.go](../tests/integration/sender_async_fifo_test.go) (порядок [1..5]; ретраи головы → [1,1,2,3,4,5] + DLQ), [sender_async_multiinstance_test.go](../tests/integration/sender_async_multiinstance_test.go) (partitions=2/instances=2). Юниты маппинга HandleResult→Commit — [consumer_test.go](../internal/sender/adapter/in/kafka/consumer_test.go) |

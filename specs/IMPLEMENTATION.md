...
| **§70: несколько нод Nexus на одном ClickHouse** | ⛔ §70 | **ТЗ зафиксировано, реализация не начата.** Раздел — [70-multi-instance-clickhouse.md](sections/70-multi-instance-clickhouse.md), ветка `feature/multi-instance-clickhouse`. Нода = самостоятельное развёртывание со своими PostgreSQL/Redis/Kafka и общим ClickHouse. Ключевые решения: `instance.id` (пусто = текущая нода, иначе 1–8 символов; тестовая нода — `kz`) → базы `nexus_<id>_<slug>`; маркер владения `__nexus_owner` (`TinyLog` — намеренно не MergeTree, иначе виден в «Orphans» и удаляем кнопкой; захват через `CREATE TABLE` **без** `IF NOT EXISTS` = атомарная проверка-и-установка, проигравший получает код 57); гейт владения перед каждой разрушающей операцией — `DROP PARTITION` ([ch_housekeeping.go](../internal/sender/usecase/ch_housekeeping.go)), удаление бесхозной таблицы ([orphan_scanner.go](../internal/web/usecase/orphan_scanner.go)), `ALTER` §56, стартовые `ADD COLUMN`/мутации ([ensure_schema.go](../internal/platform/clickhouse/ensure_schema.go)), `CREATE`/`RENAME` ([team_provisioner.go](../internal/web/adapter/out/clickhouse/team_provisioner.go)), `DELETE` неудачных доставок, послабление `node_id=''` §61. Гейт первого запуска опирается на «свежая PostgreSQL», а не на маркер: пустой `instance.id` неотличим у двух легаси-нод. Формат `teams.ch_database` релаксируется до `{0,40}` — иначе суффикс не оставляет места длинному слагу. Метка метрик — `nexus_instance`, не `instance` (последнее имя занимает scrape, метка была бы переименована в `exported_instance`). Межнодовая работа разрешена только через внешнюю таблицу §64, и там осознанно **и чтение, и запись** — §64 не read-only, Sender в такие таблицы пишет. Redis/Kafka/Prometheus между нодами не разделяются (pub/sub-каналы Redis глобальны для сервера, имена топиков и группы совпадают, запросы панели агрегируют без селектора ноды) |

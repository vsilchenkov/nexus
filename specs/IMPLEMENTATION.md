...
| Отключение логирования + обрезка по символам в Sender | ✅ Phase 22.2 | [send.go](../internal/sender/usecase/send.go) (`truncateRunes`, guard на `Write`); тесты [send_test.go](../internal/sender/usecase/send_test.go) + integration [clickhouse_test.go](../tests/integration/clickhouse_test.go); **сценарий большого тела** (клиент получает полное тело, в логе усечено, checksum по полному) [clickhouse_largebody_test.go](../tests/integration/clickhouse_largebody_test.go) |

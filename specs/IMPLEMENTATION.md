...
| Отключение логирования + ~~обрезка по символам~~ в Sender (§22.2 обрезка superseded §43) | ✅ Phase 22.2 / §43 | [send.go](../internal/sender/usecase/send.go) (guard на `Write`); тесты [send_test.go](../internal/sender/usecase/send_test.go) + integration [clickhouse_test.go](../tests/integration/clickhouse_test.go) |
| **§43: жёсткий лимит размера тела — reject вместо усечения** (sync 413/502, async не доставляет; лог done=0+reason, тело не пишется, checksum по полному) | ✅ §43 | [send.go](../internal/sender/usecase/send.go) (`exceedsBodyLimit`/`oversizeReason`, ранний выход 413, 502 на ответе, CB по `upstreamHealthy`, `truncateRunes` удалён); единая точка для sync+async ([async.go](../internal/sender/usecase/async.go) → `Send`); тесты [send_test.go](../internal/sender/usecase/send_test.go) (413/502/under-limit) + integration [clickhouse_largebody_test.go](../tests/integration/clickhouse_largebody_test.go) (лимит off→полное / on→reject) |

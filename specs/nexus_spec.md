...
`node_id`. Неизвестно, общая ли таблица (счётчик не подключён или запрос упал) → прежнее, более
мягкое правило: скрыть собственные логи хуже, чем показать лишние.

### 61.2 Цена решения

Источник данных — карта «таблица → число узлов»: считается ОДНИМ запросом на инсталляцию,
кешируется на минуту, обновляется вне блокировки и только в одной горутине (дашборд считает KPI по
всем узлам параллельно), после неудачной попытки не повторяется чаще 15 секунд (иначе недоступная
база получала бы запрос на каждое обращение к логам — сбой усиливался бы трафиком интерфейса) и
переживает отмену пользовательского запроса. Нагрузка на ClickHouse не меняется.
...
`node_id = ''` (§61) на не своей таблице отключается. Сохранение узла с таблицей в чужой базе —
400. Отбор своих таблиц идёт по `teams.ch_database` **и** маркеру, а не по маске имени.

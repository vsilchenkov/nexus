...
Правило: послабление действует только на таблице, принадлежащей одному узлу; на общей — строго свой
`node_id`.

Исключение — внешняя таблица §64. §70.4 добавил второе основание для строгости: таблица, которой эта
нода не владеет (нет маркера `__nexus_owner`). Для внешней таблицы оно неприменимо — маркера у чужой
БД нет и быть не может, а весь поток постороннего писателя идёт с `node_id = ''`, поэтому строгий
фильтр прятал ВСЁ содержимое таблицы (боевой инцидент: пустой журнал при 10 222 523 записях).
На таблице с `external_table = true` отсутствие владения строгость не включает; общая таблица
остаётся строгой и в этом случае.

Деградация — строгость включают только доказанные факты: если факты о таблице неизвестны (порт не
подключён или запрос упал), действует прежнее, более мягкое правило (скрыть собственные логи хуже,
чем показать лишние). Одинаково для обоих фактов — и «сколько узлов на таблице», и «помечена ли
внешней»: правило, строгое наполовину, превращает недоступность PostgreSQL в пропажу логов.
...
**Пятая точка — чтение не гейтится владением (§70.4).** Гейт владения опирается на маркер
`__nexus_owner`, которого у чужой БД быть не может, поэтому для внешней таблицы «не наша» — не
признак соседней ноды, а норма. Так как весь поток внешнего писателя идёт с `node_id = ''`, включённая
гейтом строгая атрибуция прятала всё содержимое таблицы (боевой инцидент: пустой журнал и нулевые KPI
при миллионах записей). На `external_table = true` строгость не включается (правило — §61.1). Границы:
разрушающие операции не смягчены (`DeleteFailed` по-прежнему упирается в `AssertOwnsTable` и на чужой
таблице отказывает); общая таблица остаётся строгой (счётчик §61 срабатывает раньше признака
внешности), включая смешанную; признак берётся из своей PostgreSQL — послабление включает оператор,
пометивший узел внешним, а не эвристика по состоянию ClickHouse.
...
`RENAME TABLE`/временной таблицей проверки шаблона, `DELETE` неудачных доставок; послабление
`node_id = ''` (§61) на не своей таблице отключается — **кроме таблиц, помеченных `external_table`
(§64)**: там «не наша» не отличает соседнюю ноду от постороннего писателя, и строгий фильтр прятал
всё содержимое таблицы. Смягчена только видимость: `DELETE` на чужой таблице гейт по-прежнему
запрещает. Сохранение узла с таблицей в чужой базе — 400. Отбор своих таблиц идёт по
`teams.ch_database` **и** маркеру, а не по маске имени.
...
записи со своим `node_id` (посторонний писатель проставить его не может); если такую таблицу делят
≥2 узла Nexus, послабление снимается и каждый видит только свои записи.

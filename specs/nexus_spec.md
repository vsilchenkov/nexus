...
- **GET-реинъекция — без тела**: поле тела скрыто, пустой `request` в логе не считается «тело не сохранялось» (у GET тела нет by design; ошибка 422 `replay.body_unavailable` остаётся только для не-GET методов). Эффективный метод в UI зеркалит выбор бэкенда: `node.incoming_method`, для ANY — глагол лога `http_method`, fallback POST.

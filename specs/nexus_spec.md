...
**UI.** Форма узла — read-only блок **внутри** карточки «Предпросмотр», под схемой маршрута за
разделителем: `Команда: <имя>` + для сохранённого узла даты §63 как на «Конфиге» («Создано» всегда,
«Обновлено» только при `updated_at ≠ created_at`, автор при пустом значении не выводится; на форме
создания дат нет). Отдельный блок над карточкой выбивался визуально и опускал предпросмотр — фидбэк
пользователя. Ключи i18n `node.fields.team`, `common.created_at`/`updated_at`/`author`. Вкладка
«Конфиг» — только имя команды, убраны `(slug)` и UUID-фолбэк. Имя недоступно → «—» (сырой

...
непустой — преф в контексте команды (перекрывает глобальный). `UNIQUE NULLS NOT DISTINCT`
(PostgreSQL ≥ 15) обязателен: иначе строки с `NULL` считаются различными, `ON CONFLICT` не
срабатывает и upsert превращается в append. Два FK намеренно: составной на `user_teams` (инвариант

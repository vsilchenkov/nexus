...
**UI.** Форма узла — read-only строка `Команда: <имя>` **внутри** карточки «Предпросмотр» (первой
строкой после заголовка; отдельный блок над карточкой выбивался визуально и опускал предпросмотр —
фидбэк пользователя). Ключ i18n `node.fields.team`. Вкладка «Конфиг» — только имя, убраны `(slug)` и

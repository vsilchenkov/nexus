...
**UI.** Форма узла — read-only строка `Команда: <имя>` вверху правой колонки (сайдбар), над
«Предпросмотром» (ключ i18n `node.fields.team`). Вкладка «Конфиг» — только имя, убраны `(slug)` и

...
размеры в `[]TopicInfo` после ответа Kafka Admin и **до** записи в Redis-кеш: запрос уходит раз в
TTL кеша (30 с), кеш хранит целостный снимок, а размер меняется достаточно медленно, чтобы
«свежесть» не имела значения.
...
Колонка «Размер» существует с §31 и меняется минимально: значение форматируется `fmtBytes`, а
нулевой размер рисуется прочерком **с подсказкой** (`title`) «Размер недоступен: на брокере не
настроен JMX-экспортёр». Без подсказки прочерк читается как «топик пуст», хотя означает
«источника нет».
...
│ nexus.async.dlq    4         1            12.6 KB   238           …    │
│ nexus.logs.retry   4         1            —         0             …    │
│                                           ↑ title: «Размер недоступен: │
│                                             на брокере не настроен     │
│                                             JMX-экспортёр»             │
└────────────────────────────────────────────────────────────────────────┘
```

Контракт API не меняется: поле `size_bytes` в ответе `GET /api/kafka/topics` было с самого начала
и просто перестаёт быть всегда нулевым.

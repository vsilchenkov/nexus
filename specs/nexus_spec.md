...
URL в таблице логов обрезается по ширине, а нативная подсказка не позволяла скопировать адрес.
Ячейка обёрнута в наводимую подсказку UI-кита с полным URL и кнопкой «Скопировать». Неочевидность:
подсказка рендерится в портал, но React-события всплывают по React-дереву — без остановки всплытия
клик по кнопке доходил бы до обработчика строки и сворачивал запись.

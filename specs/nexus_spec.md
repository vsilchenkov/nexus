...
**71.5 Поведение UI.** Резолв: преф команды → глобальный преф → системные 24ч. Принудительного
сброса периода при переключении команды нет: период производен от URL, поэтому при чистом URL
применяется дефолт новой команды сам, а явно выбранный (`?range=30d`) переживает переключение.
Прямая ссылка уважается и при первой загрузке. Пока префы не загружены — плейсхолдер вместо
переключателя, запрос метрик не уходит (иначе лишний запрос с 24ч на каждый заход). Прежнее
localStorage-значение разово мигрирует в глобальный преф и удаляется из браузера (механизм убрать
через два релиза).

**71.6 Границы.** Выбранный вручную период держится, пока не выбран период, равный дефолту команды
(тогда `range` уходит из URL), либо пока не закрыта вкладка (зеркало §54 в `sessionStorage`).
Пока дефолт команды не загружен, период пишется в URL всегда (модуль фильтров получает `null`
вместо дефолта): иначе правка другого фильтра в это окно стёрла бы явно выбранный `range=24h`.
Ошибка сохранения дефолта пользователю не показывается — оптимистичное обновление откатывается.

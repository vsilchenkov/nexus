...
3. Sender-consumer **не отправляет** сообщения paused-узлов: получив сообщение из Kafka, он перечитывает статус узла и, если `paused`, **переобрабатывает это же сообщение на месте** (retry-in-place) с паузой `pausedRetryAfter` (30 секунд), не коммитя offset, — до тех пор, пока узел не выйдет из паузы или не остановится сервис. Сообщения накапливаются в топике до снятия паузы.

   **Почему именно retry-in-place, а не «пропустить и вернуться позже».** kafka-go не передоставляет незакоммиченное сообщение тому же reader'у: курсор уже сдвинут, а commit следующего сообщения той же партиции прокатывает committed offset мимо пропущенного — оно теряется навсегда. Плата за корректность — head-of-line blocking: paused-узел в голове партиции задерживает соседние сообщения этой партиции (партиционирование идёт по `node_path`, поэтому в одну партицию могут попадать разные узлы). Подробности и репро-тесты — раздел 4.37 IMPLEMENTATION.md.

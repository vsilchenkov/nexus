...
> **Пересмотрено §52:** булево значение ключа заменено трёхсостояньем
> (`"0"`=ok, `"1"`=down, `"2"`=degraded — legacy `"1"` читается как down);
> порты переименованы в `SetLastOutcome`/`GetLastOutcomes`. Механика
> персистентности и ключ не изменились. См.

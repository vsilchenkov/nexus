...
## 50.4 Circuit breaker не открывается по 4xx

Боевой инцидент (узел `site/push`, 15.07.2026): рассылка пушей по графику попадала на протухшие
FCM-токены → push-ms отвечал `422 {"error":{"message":"NotRegistered"}}`. Breaker считал отказом всё
не-2xx ([send.go](../../internal/sender/usecase/send.go) `upstreamHealthy`), 5 подряд 422 открывали его
(порог 5, cooldown 30с — [sender/app.go](../../internal/sender/app.go)), и следующие 30 секунд **все**
пуши узла — включая валидные токены — отбивались `circuit_breaker_open` (503, без вызова upstream) и
терялись (sync-узел без ретраев). Пробный запрос после cooldown снова попадал на плохой токен —
breaker переоткрывался волнами (243 отбитых запроса за 2.5 часа).

**Фикс:** нездоровье внешнего узла = транспортная ошибка (timeout/refused) **или 5xx**. Любой ответ
`< 500` — узел жив и отвечает: 4xx — ошибка данных/клиента, breaker по ней НЕ открывается (и в
half_open такой ответ закрывает breaker). В лог записи 4xx по-прежнему идут с `done=false` и
`reason="HTTP 4xx"` — меняется только сигнал для breaker'а. Единственная точка решения —
`SendUsecase.Send` (sync и async оба идут через неё, §43-rev).

Примечание команде-владельцу узла: `NotRegistered` — законный ответ FCM «токен не зарегистрирован»;
эти 422 лечатся чисткой протухших токенов на стороне отправителя, шина их не ретраит.

...
> Схема выше — исходная (v1). Контракт с тех пор рос аддитивно; актуальный источник правды —
> [/proto/sender/v1/sender.proto](../../proto/sender/v1/sender.proto). Добавлены, в частности:
> поля логирования (`clickhouse_table`, `log_*_body`, `logging_enabled` — §22), ограничение тела
> (`max_body_size*` — §42/§43), `node_id` (§37), `request_path` (§39), `attempts`/`duration_ms` в
> ответе, а также **`dry_run` (§55)**.
>
> **`bool dry_run` (§55).** Помечает вызов как тестовый (dry-run из UI). Sender при нём:
> **не пишет** лог в ClickHouse (двойной гейт с `logging_enabled`), **не пишет** метрики §6 и gauge
> исхода §41/§52, **не персистит** статус узла в Redis §46 и **не трогает circuit breaker**
> §9.5/§50.4. Без этого флага тест конфига по мёртвому адресу покрасил бы живой узел в Down и открыл
> его breaker, отдавая 503 боевому трафику. Подробности и порядок деплоя (Sender → Web) — §55.

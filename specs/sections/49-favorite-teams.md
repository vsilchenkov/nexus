...
- **Звезда** возле каждой команды в списке переключателя добавляет/убирает её из избранного.
  Элемент выбора команды и кнопка-звезда — соседние элементы (не вложенные): звезда не переключает
  команду. С §89.3 элемент выбора — ссылка, поэтому «не вложенные» стало ещё и требованием
  валидности разметки (кнопка внутри `<a>` недопустима).
...
- **Drag-and-drop** внутри секции меняет порядок (библиотека `@dnd-kit`). Порог активации 6px —
  клик не превращается в 0px-драг; click, прилетающий после drop, гасится флагом. С §89.3 гашения
  флагом мало: строка стала ссылкой, и без `preventDefault` браузер ушёл бы по href — то есть
  перетаскивание заканчивалось бы переходом.
- **Открытие в новой вкладке** (§89.3): строки — настоящие ссылки на `/?team=<slug>` (режим «Все
  команды» — на `/?team=*`), поэтому работают контекстное меню браузера, Ctrl/Cmd+клик и средняя
  кнопка. Обычный левый клик по-прежнему обрабатывает приложение.

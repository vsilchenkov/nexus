...
До выполнения ALTER (деплой-окно):

- **Запись**: INSERT Sender'а по такой таблице падает → записи буферизуются в Kafka-retry
  (§38) и доигрываются после ALTER. Потери нет.
- **Чтение списка/count**: SELECT с `client_host` падает — чтение логов узла временно
  недоступно (транзитное состояние, закрывается SQL из release notes).
- **Facet `/logs/client-hosts`**: мягкая деградация — `isMissingColumnErr` (CH-коды 10/16/47 +
  имя колонки) → пустой список + Debug, без 500/Sentry-флуда.
- **Verify** (`POST /api/ch-tables/verify`): показывает `Missing: ["client_host"]` — сигнал
  владельцу выполнить ALTER.

...
| 32 | [32-loop-protection.md](32-loop-protection.md) | Защита от зацикливания — служебный hop-счётчик `X-Nexus-Hops` (инкремент на каждом проходе через шину, обрыв при `receiver.max_hops`, дефолт 5 → **508 Loop Detected**, sync+async), self-reference валидация `target_url` при сохранении узла (`web.self_ingress_hosts`), метрика `nexus_receiver_loop_detected_total` |

...
Все per-node запросы `LogReaderCH` фильтруют **`node_id = ?`**: `ListSince`
...
**Записи с пустым `node_id` узлу не принадлежат.** Изначально фильтр был мягким
(`OR node_id = ''`) ради записей, писавшихся до появления колонки: они неразличимы по владельцу и
трактовались как принадлежащие любому узлу таблицы. §61 сузил послабление до не-общих таблиц, а с
версии 1.22.5 оно снято совсем — кроме **внешних таблиц §64**, где посторонний писатель не
проставляет `node_id` в принципе и пустое значение имеет каждая строка (полное правило — §61.1).

Что это значит на практике: записи, сделанные до 2026-06-19 (появление колонки), у узла больше не
видны; они остаются в таблице и истекают по её TTL. Скорость роли не играла — замер показал, что
мягкий и строгий варианты читают одинаковый объём (§61.2).
...
- **Почему изначально было `OR node_id = ''`:** обратная совместимость со старыми записями без миграции
  их значений (`ALTER … UPDATE` на больших таблицах дорог). Компромисс держался, пока такие записи
  составляли историю узлов; в 1.22.5 послабление снято (см. §37.4 и §61.1) — своей истории оно узлу не
  добавляло, а на общей таблице показывало чужое.

...
>
> **С 1.34.0 (§97) конфиг задаёт ПОТОЛОК, а не действующий лимит.** `receiver.max_body_bytes` и
> `receiver.max_async_body_bytes` говорят, сколько шина способна переварить (под них настроены
> nginx, gRPC, Kafka и память сервисов), а сколько разрешено сегодня — задаёт администратор в
> «Настройки → Общие» (по умолчанию 100 МиБ) и меняет без рестарта. Всё описанное ниже про 413/502
> в силе; изменилось только то, откуда берётся сравниваемое число:
> `clamp(настройка ?? 100 МиБ, 1 КиБ … потолок)`. Подробности — §97.

...
5. **Правило впредь (внесено в правила разработки — CLAUDE.md §1 и чек-лист §10):** в НОВОМ коде
   debug-логирование закладывается сразу — в местах, где могут появиться проблемы или потребуется
   анализ, и в нечётких/опасных/сложных неявных местах. Типовые варианты добавления: (а) след
   решения на ветвлении (`route: node resolved`, выбранная ветка/источник + ключевые атрибуты);
   (б) Debug на каждом тихом `return` / проглоченной best-effort-ошибке (`write-back skipped`,
   fail-open breaker'а) — молчаливые деградации не должны быть невидимыми; (в) исход каждого
   внешнего вызова (status, attempts, duration_ms, reason); (г) кеш-события (miss/источник/
   инвалидация с реальным ключом). Ограничения те же: структурные атрибуты без секретов/тел,
   URL без query, дорогие атрибуты за проверкой уровня, без дублирования info/warn.

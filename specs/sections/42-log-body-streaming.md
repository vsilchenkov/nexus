...
### 42.10. Истинные размеры тел (`request_size`/`response_size`)

Две колонки лог-таблиц ClickHouse с **истинными размерами тел в байтах**: `request_size Int64` (тело
исходящего запроса) и `response_size Int64` (тело ответа), `DEFAULT 0`, в конце обязательной схемы
(`RequiredLogColumns`, после `node_id`).

**Семантика.** Размер считается Sender'ом в `Send` по **полному** телу (`len(in.Body)` /
`len(resp.Body)`) — до усечения лог-копии по `max_body_size` и независимо от флагов
`log_request_body`/`log_response_body` (тот же инвариант, что у checksum). Отличие от полей API
`request_len`/`response_len` (§42.4): те — **руны сохранённой (возможно усечённой) копии**
(`lengthUTF8`), новые — **байты реального тела**. `0` — тела нет / транспортная ошибка (breaker,
`resp == nil`) / `TooLarge` §43 (тело не дочитано, размер неизвестен).

**Миграция.** На старте Web и Sender (порядок деплоя не гарантирован):

1. `EnsureBodySizeColumns` — идемпотентный `ALTER … ADD COLUMN IF NOT EXISTS` по образцу §37/§39.
2. `BackfillBodySizes` — разовое заполнение исторических строк значением `length()` (байты)
   **сохранённой копии** тела — для усечённых записей это нижняя граница (истинный размер до усечения
   не восстановим). Guard-`count()` пропускает таблицу без подходящих строк, поэтому мутации
   (`ALTER … UPDATE`, асинхронные) планируются только один раз; ошибки — log+continue.

Rolling-совместимость: старый Sender пишет явным списком колонок (новые получают DEFAULT 0); Kafka
retry-конверт (§38) — JSON `LogRecord`, старые записи без полей десериализуются в 0.

**API.** `request_size`/`response_size` в `LogRecordDTO` возвращаются **всегда** (list, stream,
detail; без `omitempty`) — колонки-числа дешёвые, в отличие от тел, которые список не читает.

**UI.** В таблице логов — компактная колонка «Ответ» (en `Resp`, полное имя в тултипе заголовка):
размер тела ответа (`fmtSize`, локализованные единицы `logs.size_units` — «12.0 КБ» / "12.0 KB"),
`—` при 0 — сразу видно, есть ли ответ. В раскрытой записи размеры выводятся в скобках рядом с
заголовками панелей: «Запрос (12.0 КБ)» / «Ответ (34.5 КБ)»; при 0 скобки не показываются.

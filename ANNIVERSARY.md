...
Один из узлов обмена 2 сентября: доставка встала в 11:08, пакет рос с 46 до 62 МБ за смену (+2 МБ в час) и
...
неудачные» — и приёмник, обмен с 1С, принял 15 дублей. Корень:
...
**Курсор, который двигается только по эху.** Другой узел: клиент-1С слал около одиннадцати
...
**Обмен 1С → 1С и усечённое тело.** 31 августа на узле такого обмена приёмник какое-то время отвечал
...
- **[§82](./specs/sections/82-sync-ceiling-and-async-ingress.md)** — Потолок длительности sync-вызова и гейт async-входа (развивает §3/§9.3/§78/§81.2). Продолжение боевого разбора 06…

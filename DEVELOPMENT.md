...
- **Виртуализация журнала узла (§98.6)** — новая npm-зависимость **`@tanstack/react-virtual`**
  (тот же вендор, что react-query). Коммитьте `package.json` вместе с `package-lock.json`.
  Что важно при правках `LogsTab`: единица виртуализации — `<tbody>` на запись (у раскрытой строки
  §77.1 тел два, и они обязаны измеряться целиком), ключ элемента — id записи, а не индекс
  (tail-poll дописывает записи СВЕРХУ). Нулевые измерения контейнера и строки подменяются оценками:
  в jsdom все размеры нулевые, и без этого тесты журнала проверяли бы пустую таблицу, молча зеленея.
  Считать загруженные записи по числу строк в DOM больше нельзя — для этого есть счётчик
  «Показано N».
- **Группировка узлов (§99)** — новых зависимостей и шагов сборки не добавляет, миграция 0042
  аддитивная. Что важно при правках: справочник групп ГЛОБАЛЬНЫЙ, и его queryKey `"node-groups"`
  обязан оставаться в `TEAM_INDEPENDENT_KEYS` (`lib/teams.ts`) — иначе смена команды перезапрашивает
  данные, которые от команды не зависят. Раскладка по группам живёт в чистой `lib/nodeGroups.ts`:
  она НЕ сортирует, а разбивает уже отсортированный список, поэтому сортировку Overview (и живую, и
  замороженную §86.10) трогать не требуется — при доработках это свойство надо сохранять. В общий
  `test/setup.ts` добавлена заглушка `Element.prototype.scrollIntoView`: jsdom его не реализует, а
  cmdk зовёт — без неё падает любой тест, открывающий комбобокс справочника (§24 и §99 одинаково).

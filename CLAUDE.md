...
**Пре-мерж ревизия: перед слиянием ветки в `dev` — перечитать и проанализировать ВЕСЬ diff.**
Непосредственно перед merge прогони `git diff dev...HEAD` и перечитай каждое содержательное
изменение свежим взглядом — не «свой код, я его помню», а как чужой PR: контракты (что функция
обещает в godoc — она это реально выполняет на всех ветках?), утечки чувствительных данных
(тексты ошибок стандартной библиотеки могут нести куски входа — например, `textproto` вклеивает
сырую строку тела в «malformed MIME header»), обходные пути записи мимо новой логики (кто ещё
строит тот же артефакт?), мёртвый код и no-op тесты. Найденное — исправить отдельным коммитом
`Phase N.M: пре-мерж ревизия — <что найдено>`, добавить регрессионный тест (красный на старом
коде), перегнать тесты/линт и **показать пользователю, что именно исправлено**. Урок §68:
ревизия перед merge нашла утечку содержимого тела в ClickHouse через текст ошибки парсера —
все юнит/integration/стенд-гейты этот баг пропустили, его поймало только перечитывание diff'а.

...
export type NodeTeam = {
  ...
  team_slug: string;
  team_name: string;
  // team_external_url — §89.4: внешняя ссылка команды-ВЛАДЕЛЬЦА узла. Едет
  // здесь, а не резолвится по /api/me/teams: вкладка «Конфиг» обязана показать
  // адрес команды узла, а её в членствах вызывающего может не быть вовсе —
  // тот же случай, ради которого §58 и завёл этот резолвер.
  team_external_url: string;
};

...
export const TEAM_INDEPENDENT_KEYS = new Set([
  ...
  "node-search",
  "search-history",
  // §99: справочник групп узлов. GET /api/node-groups team-фильтра не имеет —
  // группы намеренно глобальные (одна прикладная группа встречается в разных
  // командах, и в сквозном режиме §86 команднозависимый справочник дал бы на
  // экране несколько одноимённых). Инвалидация на смене команды дала бы рефетч
  // тех же данных ровно в момент, когда список узлов перестраивается.
  "node-groups",

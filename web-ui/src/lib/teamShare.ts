...
//                 ВНИМАНИЕ: исключён именно /logs, а не всё поддерево. Журнал
//                 отказов /logs/rejected (§94) team-scoped, и ссылка на
//                 конкретный отказ (§98.3) без параметра открывалась бы у
//                 получателя в его команде — то есть «не найдено».
...
// же команде.
const TEAM_PARAM_ROUTES = new Set(["/", "/kafka", "/audit", "/logs/rejected"]);

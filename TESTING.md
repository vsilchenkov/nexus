...
| `retention_bytes` | `42949672960` (**40 ГиБ / партицию**) | Предохранитель диска: Kafka чистит старые сегменты при превышении, даже если младше `retention_ms`. **Пер-партиция** — суммарно по топику = 40 ГиБ × `partitions` (при 4 партициях ≈ 160 ГиБ). Держать заметно выше пикового backlog, иначе при отставании удалятся непрочитанные (потеря); мониторить lag. |
...
### Масштабирование партиций nexus.async: когда и как

Потолок параллелизма обработки = `min(consumer.instances, partitions)` — каждая горутина-reader
Sender'а (`ConsumerGroup`, `instances`) обрабатывает по одному сообщению за раз, активных readers не
больше числа партиций. Дефолт: `partitions=4`, `instances=4`.

**Сигналы, что пора увеличивать партиции (мониторить в Prometheus, §31):**

1. **Consumer lag устойчиво растёт** и не возвращается к ~0 в спокойные периоды (главный сигнал).
   Смотреть тренд, не всплеск. Порог тревоги ≈ `поток(msg/с) × целевое_время_дренажа`.
2. **Все партиции читаются** (активных readers = `partitions`) **и** `nexus_kafka_in_flight` держится
   у `instances` (readers постоянно заняты) — параллелизм упёрт в число партиций.
3. **End-to-end задержка async растёт** (время `received_at`→запись в CH увеличивается).

**Порядок диагностики (отличить «мало партиций» от «мало consumer'ов» / «тормозит апстрим»):**

- Если активных readers < `partitions` → сначала поднять `consumer.instances`/реплики Sender
  (партиций хватает, просто не все читаются) — дёшево, без `--alter`.
- Если внешние апстримы медленные (высокая latency `send`) → партиции не помогут, чинить апстрим/таймауты.
- Если readers = `partitions`, апстрим здоров, а lag растёт → **увеличить партиции**:
  `kafka-topics --alter --topic nexus.async --partitions N` (только вверх, вниз нельзя),
  и синхронно поднять `consumer.instances`/реплики под них.

Оценка нужного числа (закон Литтла): `partitions ≈ пиковый_поток(msg/с) × средняя_latency_обработки(с) × 1.5`.
Планировать заранее по тренду, а не по факту затыка. **Перекос:** если lag сидит на 1–2 партициях
(«горячий» узел с большим потоком — `key=node.Path` держит его в одной партиции ради порядка),
общее увеличение партиций не поможет — нужен отдельный топик/пересмотр шардинга для этого узла.

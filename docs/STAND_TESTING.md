...
- **#3.1 Роль «Оператор» (§87).** Завести пользователя с ролью «Оператор» (в диалоге
  роли идут сеткой 2×2 — четыре карточки, подсказки не рвутся; в таблице у роли
  янтарный чип). Под ним: вкладка «Очередь» рабочая целиком и кнопка повтора в
  логах активна (см. чек-лист «Очередь» ниже), а кнопок «Новый узел»,
  «Редактировать», «Копировать», «Тестовый запрос» НЕТ и прямой заход на
  `/nodes/<id>/edit` уводит на просмотр. Пункт меню «Аудит» есть; вкладок
  Settings → Allowed Hosts / Users / Teams и общих настроек нет.
  Затем войти менеджером и наблюдателем и убедиться, что их поведение не
  изменилось — сдвиг рангов затрагивает все роли сразу.
...
- [ ] Под viewer вкладка открывается без 403 (failed-view виден; секции управления скрыты).
- [ ] Под **operator** (§87) вкладка «Очередь» рабочая ЦЕЛИКОМ: KPI «Ожидают отправки» с числом
      (а не «—»), кнопки «Пауза»/«Отключить»/«Возобновить», удаление сообщения, обе очистки,
      «Повторить все сейчас», «Отправить повторно из логов», «Снять защиту» на карточке breaker.
      Каждую нажать и смотреть панель СЕТЕВЫХ ЗАПРОСОВ: должен уходить 200, а не 403.
- [ ] Консоль без ошибок.

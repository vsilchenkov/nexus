...
- [ ] «Возобновить узел» → статус «Активен», прежние кнопки. Накопленные сообщения уходят сами,
      **без перезапуска Sender'а** (retry-in-place, см. 4.37 IMPLEMENTATION.md).
- [ ] Помнить при диагностике: пока узел на паузе, его сообщение стоит в голове своей Kafka-партиции
      и задерживает соседние сообщения этой партиции — в том числе ДРУГИХ узлов (партиционирование по
      `node_path`, хеш может совпасть). Это ожидаемое поведение: гарантия «ничего не потеряно» важнее
      пропускной способности заблокированной партиции. Симптом «второй узел молчит, пока первый на
      паузе» — не баг.

# Grafana dashboards для DataBus
...
- `service` — мульти-селектор сервисов (receiver/sender/web), вытягивается через
  `label_values(databus_requests_total, service)`.

### Импорт через UI

1. Открыть Grafana → Dashboards → New → Import.
2. Upload JSON file: выбрать `deploy/grafana/databus.json`.
...
- `up{job=databus-*} == 0` — недоступность сервиса.
- `5xx-rate > 5%`, `p95 > 200ms` — деградация sync-пути по §9.1.
- `databus_kafka_lag > 10k`, `clickhouse_errors_total > 0`, `dropped > 0` —

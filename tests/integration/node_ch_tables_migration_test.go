...
func TestNodeRepo_ListClickHouseTables_AllTeams_E2E(t *testing.T) {
	...
	// §64/§70.4: зеркальный запрос ExternalCHTables проверяется на ЭТОЙ ЖЕ
	// фикстуре — у неё уже есть все три интересных случая (внешняя, обычная,
	// смешанная), а поднимать ради второго SELECT'а ещё один контейнер незачем.
	// По этому множеству read-path логов отличает «таблица чужая, потому что ею
	// владеет соседняя нода» (строгий фильтр §70.4) от «чужая, потому что так
	// задумано» (послабление §64 — иначе логи внешней таблицы не видны вовсе).
	ext, err := nodeRepo.ExternalCHTables(ctx)
	require.NoError(t, err)
	require.Contains(t, ext, "external_db.audit_log",
		"§64: таблица внешнего узла обязана быть помечена — на ней снимается гейт владения")
	require.NotContains(t, ext, "nexus_default.in_default",
		"обычная таблица не внешняя — гейт §70.4 на ней остаётся в силе")
	require.NotContains(t, ext, "nexus_acme.in_acme")
	require.Contains(t, ext, "nexus_default.mixed",
		"смешанная таблица попадает в ОБА списка, и это не противоречие: её делят ≥2 узла, "+
			"а такая читается строго в любом случае — до признака внешности сравнение не доходит")
	require.Len(t, ext, 2, "помечены ровно две таблицы — послабление не расползается на остальные")
